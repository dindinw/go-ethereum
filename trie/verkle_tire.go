...
func (t *VerkleTrie) GetAccount(key common.Address) (*types.StateAccount, error) {
	var (
	...
	versionkey := t.pointCache.GetTreeKeyVersionCached(key.Bytes())
...
func (t *VerkleTrie) UpdateAccount(key common.Address, acc *types.StateAccount) error {
	var (
		err            error
...
func (trie *VerkleTrie) UpdateStorage(address common.Address, key, value []byte) error {
	k := utils.GetTreeKeyStorageSlotWithEvaluatedAddress(trie.pointCache.GetTreeKeyHeader(address.Bytes()), key)
	var v [32]byte
	copy(v[:], value[:])
...
func (t *VerkleTrie) DeleteAccount(key common.Address) error {
	var (
		err    error
		values = make([][]byte, verkle.NodeWidth)
		stem   = t.pointCache.GetTreeKeyVersionCached(key[:])
...
// https://github.com/ethereum/go-ethereum/commit/22c3ad1d1270321c79c7798a5b44333b5ad46ad8
...
func (trie *VerkleTrie) Commit(_ bool) (common.Hash, *NodeSet) {
	...
	if !ok {
		...
		//errors.New("unexpected root node type")
	}
	nodes, err := root.BatchSerialize()
	if err != nil {
		return common.Hash{}, nil
		//fmt.Errorf("serializing tree nodes: %s", err)
	}
	for _, node := range nodes {
		if err := trie.db.diskdb.Put(node.CommitmentBytes[:], node.SerializedBytes); err != nil {
			return common.Hash{}, nil
			// fmt.Errorf("put node to disk: %s", err)
		}
	}
